...
public final class TranscriptionQueueController {
    ...
    private func startNextIfNeeded() {
        ...
        queuedEntryIDs = queuedRequests.map(\.entryID)
        activeJob = ActiveTranscriptionJob(
    ...
    private func process(_ request: QueuedTranscriptionRequest) async {
        ...
        guard let provider = providersByID[request.providerID] else {
            fail(request.entryID, message: "The requested transcription provider is unavailable.", originalEntry: originalEntry)
            return
        }
        ...
        entry.providerName = request.providerName
